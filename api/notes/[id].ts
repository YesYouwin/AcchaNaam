...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (req.method === 'PUT') {
      ...
      return res.status(200).json({
        note: {
          ...
          collaborators: data.collaborators === null || data.collaborators === undefined ? undefined : Number(data.collaborators),
    ...
    if (req.method === 'DELETE') {
      const { error } = await supabase.from('notes').delete().eq('id', id).eq('client_id', clientId);
      if (error) return res.status(500).json({ error: error.message });
